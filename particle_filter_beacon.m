...
opt = jsondecode(fileread('pladema.json'));
min_x = 0;
max_x = plano_ancho;
min_y = 0;
max_y = plano_alto;

%Update initial point to match map
...
hold on;
plot(x_P/opt.width*plano_ancho,0, 'o','MarkerFaceColor','blue' )
hold off;
...
for t = 1:T
    ...
    %imagesc([min_x max_x], [min_y max_y], pladema)
    %hold on;
    %plot(plano_ancho/2, plano_alto/2, 'o','MarkerFaceColor','green')
    plot(x_P/opt.width*plano_ancho,t/T*plano_alto, 'o','MarkerFaceColor','blue' )
    plot(x/opt.width*plano_ancho,t/T*plano_alto, 'o','MarkerFaceColor','red' )
    plot(x_est/opt.width*plano_ancho,t/T*plano_alto, 'o','MarkerFaceColor','green' )
    x_P
    z
    hold off;
    %}
...
fit_in_space_factor = (max(x_out)-min(x_out)) / opt.width * plano_ancho
no_fit =  ( 1 / abs(max(x_out) - min(x_out)) ) * plano_ancho
t = 0:T;
figure(1);
hold on;
%plot(x/opt.width*plano_ancho,t/T*plano_alto, 'o','MarkerFaceColor','red' )
plot( (x_out + abs(min(x_out)) ) * no_fit, t/T*plano_alto, '.-b', (x_est_out + abs(min(x_est_out))) * no_fit , t/T*plano_alto, '-.r','linewidth',3);
hold off;
